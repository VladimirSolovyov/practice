...
<b>{%$AR_MSG.SearchModule.msg41%}</b>&nbsp;
	...
	<select onchange="window.location.href = '{%php%}echo $sLink->link{%/php%}&g='+this.value">
	...
	{%foreach key=dat_id item=row from=$alternatives%}
			...
			<option value="{%$row.id%}" {%php%}if ($_REQUEST['g']==$this->_tpl_vars['row']['id'] || strtoupper($_REQUEST['brand']) == strtoupper($this->_tpl_vars['row']['brand'])) echo "SELECTED";{%/php%}>
...
{%assign var="match_criteria" value=""%}

{%assign var="univers" value=""%}

{%assign var="oem" value=""%}

{%assign var="article" value=""%}

{%assign var="show_article" value=1%}

{%assign var="show_brand" value=1%}
...
//приведем бренд к единому регистру

$this->_tpl_vars['row']['brand'] = mb_strtolower($this->_tpl_vars['row']['brand']);
...
{%if $article != $row.parsed_article || $row.brand != $brand%}
    ...
    {%assign var="article" value=$row.parsed_article%}

    {%assign var="brand" value=$row.brand%}

    {%assign var="show_article" value=1%}

{%else%}

    {%assign var="show_article" value=0%}
...
{%if $row.match_criteria != $match_criteria%}
	...
	{%assign var="match_criteria" value=$row.match_criteria%}
...
{%if $row.univers != $univers && $match_criteria == 1%}
	...
	{%assign var="univers" value=$row.univers%}
...
<tr {%if $row.sts_style!=''%}class="lg" style="{%$row.sts_style%}"{%else%}class="{%if $isProvider.provider_id == $row.provider_id && $row.provider_id>0%}provider_row{%elseif $dat_id mod 2 == 0%}even{%else%}odd{%/if%}"{%/if%} onmouseover="toggleRow(this)" onmouseout="toggleRow(this)" onclick="clickRow(this)">
	...
	{%foreach key=hdr_id item=column from=$__search_results.header%}
		...
		<td align="{%if $hdr_id == 'spare_info' || $hdr_id == 'article' || $hdr_id == 'prd_info_link'%}left{%elseif $hdr_id == 'final_price'%}right{%elseif $hdr_id == 'customer_price'%}right{%elseif $hdr_id == 'dlv_weight_tax'%}right{%elseif $hdr_id == 'price_brutto'%}right{%else%}center{%/if%}" {%if $show_article==1%}style="border-top: 1px solid #A0A0A0"{%/if%}>
		...
		{%elseif $hdr_id == 'term'%}
			...
			<span class="tovar_sclad" title="Товар на складе"></span>	
		...
		{%elseif $hdr_id == 'prd_info_link'%}
			...
			<div {%if $row.brand_full_name!=""%}title="{%$row.brand_full_name%}"{%/if%} {%if $row.oem_brand == 1%}class="web_ar_oem_brand"{%else%}class="web_ar_brand"{%/if%}>
		...
		{%elseif $hdr_id == 'remains' %}	
			...
			{%if $row.remains == 'есть' && $row.info_only == 0%}
