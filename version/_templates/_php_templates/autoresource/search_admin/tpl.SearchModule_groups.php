<? if (($SearchSetting['authUserSearchOnly'] != 1) || ($SearchSetting['cst_category_id'] >= 1) || ($SearchSetting['admin_search'] == 1)) { ?>
	...
	<? if ($__search_results && !$SearchSetting['empty_search']) { ?>
		...
		<? if (!$SearchSetting['admin_search']) { ?>
			...
			<? include(PHP_DataRender::includeTemplatePath('/autoresource/search_admin/groups/tpl.top_caption.php')); ?>
		...
		<? } else { ?>
			...
			<? include(PHP_DataRender::includeTemplatePath('/autoresource/search_admin/groups/tpl.search_results_table.php')); ?>
