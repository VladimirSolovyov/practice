...
<? if (($SearchSetting['authUserSearchOnly'] != 1) || !$SearchSetting['isGuest'] || ($SearchSetting['admin_search'] == 1)) { ?>
	...
	<? if ($__search_results) { ?>
		...
		<? if (!$admin_search) { ?>
			...
			<? include(PHP_DataRender::includeTemplatePath('/autoresource/search/classic/tpl.top_caption.php')); ?>
		...
		<? } else { ?>
			...
			<? include(PHP_DataRender::includeTemplatePath('/autoresource/search/classic/tpl.search_results_table.php')); ?>
