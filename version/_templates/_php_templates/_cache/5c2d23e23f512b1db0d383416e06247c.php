	...
	<? foreach ($CONTENT as $key => $item) { ?>
		<? if ($item['str_level'] == $level and $item['str_left'] > $left and $item['str_right'] < $right) { ?>
			...
			<li class="user-menu__item">
				<a class="user-menu__link" href="<?= $item['str_url'] ?>" <?= (!empty($item['str_metadata']['rel']) ? ' rel="' . $item['str_metadata']['rel'] . '"' : '') . (!empty($item['str_metadata']['data-height']) ? ' data-height="' . $item['str_metadata']['data-height'] . '"' : '') . (!empty($item['str_metadata']['data-width']) ? ' data-width="' . $item['str_metadata']['data-width'] . '"' : '') ?>>
